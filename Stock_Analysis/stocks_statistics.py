...
dataset["Log_Returns"] = np.log(dataset["Adj Close"].shift(-1)) - np.log(
    dataset["Adj Close"]
)
...
density["x"] = np.arange(
    dataset["Log_Returns"].min() - 0.01, dataset["Log_Returns"].max() + 0.01, 0.001
)
...
print(
    "The probability of dropping over 10% in 225 days is ",
    norm.cdf(-0.10, mu225, sigma225),
)
...
for d in drops:
    ...
    print(
        "The Probability of dropping over",
        d,
        "% in 225 days is",
        round(prob_returns, 5),
    )
...
print(
    "At significant level of {}, shall we reject: {}".format(
        alpha, zhat > zright or zhat < zleft
    )
)
...
stat, p_value, dof, expected = chi2_contingency(
    dataset[["Open", "Low", "High", "Adj Close", "Volume"]]
)
...
stat, p_value = friedmanchisquare(
    dataset["Open"], dataset["Adj Close"], dataset["Volume"]
)
...
if p_value > alpha:
    ...
    print("Different distributions (reject H0)")
stat, p_value, med, tbl = median_test(
    dataset["Open"], dataset["Adj Close"], dataset["Volume"]
)
...
stat, p_value, med, tbl = median_test(
    dataset["Open"], dataset["Adj Close"], dataset["Volume"], lambda_="log-likelihood"
)
