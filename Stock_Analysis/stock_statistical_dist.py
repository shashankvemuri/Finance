...
# Create more data
dataset["Increase_Decrease"] = np.where(
    dataset["Volume"].shift(-1) > dataset["Volume"], 1, -1
)
dataset["Buy_Sell_on_Open"] = np.where(
    dataset["Open"].shift(-1) > dataset["Open"], 1, -1
)
dataset["Buy_Sell"] = np.where(
    dataset["Adj Close"].shift(-1) > dataset["Adj Close"], 1, -1
)
...
ax = sns.distplot(
    data_binom, kde=False, color="skyblue", hist_kws={"linewidth": 15, "alpha": 1}
)
...
increase_probability = pd.DataFrame(
    {
        "probability": dataset.groupby(by="Increase_Decrease", as_index=False).size()
        / dataset.Increase_Decrease.count()
    }
).reset_index()
