...
plt.axvline(
    x=2 * stdev + mean, ymin=0, ymax=1, color="gray", alpha=0.75, linestyle="--"
)
plt.axvline(x=3 * stdev + mean, ymin=0, ymax=1, color="gray", alpha=0.5, linestyle="--")
plt.axvline(x=-stdev + mean, ymin=0, ymax=1, color="gray", alpha=1, linestyle="--")
plt.axvline(
    x=-2 * stdev + mean, ymin=0, ymax=1, color="gray", alpha=0.75, linestyle="--"
)
plt.axvline(
    x=-3 * stdev + mean, ymin=0, ymax=1, color="gray", alpha=0.5, linestyle="--"
)
...
# 3Dev Bands
data["upper_band"], data["middle_band"], data["lower_band"] = talib.BBANDS(
    data["Adj Close"], timeperiod=50, nbdevup=3, nbdevdn=3
)
