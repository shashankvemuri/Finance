    ...
    try:
        if request["equity"]["EQ"]["isOpen"] is True:
            ...
            def sell_stocks(df, df_pf, sell_list, date):
                ...
                # Filter
                df_sell_price = df_sell_price.loc[
                    df_sell_price["symbol"].isin(sell_list)
                ]
                ...
                if df_sell_price.shape[0] > 0:
                    ...
                    df_sell = pd.merge(
                        df_sell_price, df_buy_shares, on="symbol", how="left"
                    )
            ...
            # Call the function
            df_sell = sell_stocks(
                df=df, df_pf=df_pf, sell_list=sell_list, date=current_data_date
            )
            ...
            # Call the function
            df_sell_final = stock_diffs(df_sell=df_sell, df_pf=df_pf, df_buy=df_buy)
