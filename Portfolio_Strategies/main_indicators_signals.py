...
for symbol in tickers:
    ...
    close_diff = data['Adj Close'].diff().tolist()
    close_diff = close_diff[::-1]
    close_diff = mean(close_diff[:14])
    ...
    overall_signal = [round((signal_1 + signal_2 + signal_3 + signal_4 + signal_5)/5), 2]
    smas = [round(sma[0], 2)]
    emas = [round(ema[0], 2)]
    ...
    dataframe = pd.DataFrame(list(zip(ticker, overall_signal, smas, emas, closes, uppers, lowers, macds, macd_signals, rsis, obvs)), columns =['Company', 'Signal', 'SMA', 'EMA', 'Adj Close', 'Upper Band', 'Lower Band', 'MACD', 'MACD Signal' ,'Relative Strength Index', 'On Balance Volume'])
